...
<body class="min-h-screen flex flex-col justify-center items-center text-slate-100">
  ...
  <section id="inGameBoardScreen" class="hidden w-full h-screen max-h-screen flex flex-col bg-zinc-950/85 overflow-hidden">
    ...
    <!-- Main Workspace -->
    <div class="flex-1 p-1 sm:p-1.5 flex flex-col xl:flex-row items-center xl:items-stretch justify-center gap-1.5 xl:gap-3 max-w-[1650px] mx-auto w-full min-h-0 overflow-hidden relative">
      <!-- Left Column: Player Cards & Chat (Desktop / Mobile Off-canvas Drawer) -->
      <aside id="asideLeft" class="w-full xl:w-56 2xl:w-64 shrink-0 flex flex-col gap-1.5 xl:gap-2 order-2 xl:order-1 min-h-0">
        ...
        <div id="playerInfoCard" class="mobile-panel-section bg-zinc-900/95 border border-zinc-800 rounded-xl p-2 sm:p-2.5 shadow-xl shrink-0">
          ...
          <div id="playersListContainer" class="space-y-1 overflow-y-auto flex-1 pr-0.5"></div>

          <!-- Jail Actions (Aktif jika pemain sedang di penjara) -->
          <div id="jailActions" class="hidden pt-1.5 border-t border-zinc-800 space-y-1 mt-1.5 shrink-0">
      ...
      <aside id="asideRight" class="w-full xl:w-56 2xl:w-64 shrink-0 flex flex-col gap-1.5 xl:gap-2 order-3 min-h-0">
        ...
        <div id="tradingCard" class="mobile-panel-section bg-zinc-900/95 border-2 border-red-500/40 rounded-xl p-2 sm:p-2.5 shadow-xl flex flex-col gap-1 bg-gradient-to-b from-zinc-900/95 to-zinc-950/95 shrink-0">
          ...
          <button id="btnOpenTradingDesk" class="w-full py-1.5 rounded-xl btn-menu-orange text-white font-black text-xs uppercase tracking-wider transition flex items-center justify-center gap-1.5 cursor-pointer font-outfit shadow active:scale-95">
            <svg class="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
              <path d="M6.99 11L3 15l3.99 4v-3H14v-2H6.99v-3zM21 9l-3.99-4v3H10v2h7.01v3L21 9z"/>
            </svg>
